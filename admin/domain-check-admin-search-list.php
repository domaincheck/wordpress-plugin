...
class DomainCheck_Search_List extends WP_List_Table {
    ...
    */
    ...
    public static function record_count() {
        global $wpdb;
        $sql = "SELECT COUNT(*) FROM " . DomainCheck::$db_prefix . '_domains';
        ...
        function column_status( $item ) {
            ...
            switch ($status) {
                case 0:
                    ...
                    . '<img id="status-image-' . str_replace('.', '-', $item['domain_url']) . '" src="' . plugins_url('/images/icons/circle-check.svg', __FILE__) . '" class="svg svg-icon-table svg-fill-available">'
                    . ' Available</a>'
                ...
                case 1:
                    ...
                    $out = '<a id="status-link-' . str_replace('.', '-', $item['domain_url']) . '" onclick="domain_check_ajax_call({\'action\':\'status_trigger\', \'domain\':\'' . $item['domain_url'] . '\'}, status_trigger_callback);">'
                        . '<img id="status-image-' . str_replace('.', '-', $item['domain_url']) . '" src="' . plugins_url('/images/icons/ban.svg', __FILE__) . '" class="svg svg-icon-table svg-fill-taken">'
                        . ' ' . $out . '</a>';
                break;
                case 2:
                    $out = 'Owned';
                    $out = '<a id="status-link-' . str_replace('.', '-', $item['domain_url']) . '" onclick="domain_check_ajax_call({\'action\':\'status_trigger\', \'domain\':\'' . $item['domain_url'] . '\'}, status_trigger_callback);">'
                            . '<img id="status-image-' . str_replace('.', '-', $item['domain_url']) . '" src="' . plugins_url('/images/icons/flag.svg', __FILE__) . '" class="svg svg-icon-table svg-fill-owned">'
                            . ' ' . $out . '</a>';
                break;
                default:
