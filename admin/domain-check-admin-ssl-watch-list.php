 ...
 * @since 0.1.0
 */
class DomainCheck_SSL_Watch_List extends WP_List_Table {
	/**
	* Constructor.
	*/
	...
	*/
	public static function get_domains( $per_page = 50, $page_number = 1 ) {
		global $wpdb;
		$sql = 'SELECT * FROM ' . DomainCheck::$db_prefix . '_ssl WHERE domain_watch > 0';
	...
	*/
	public static function record_count() {
		global $wpdb;
		$sql = "SELECT COUNT(domain_id) FROM " . DomainCheck::$db_prefix . '_ssl WHERE domain_watch > 0';
	...
	*/
	function column_domain_url( $item ) {
		...
		$out .= ' <a href="?page=domain-check-ssl-profile&domain='.$item['domain_url'].'"><strong>'.$item['domain_url'].'</strong></a>&nbsp;&nbsp;'
					. '<a href="//' . $item['domain_url'] . '" target="_blank" class="hidden-mobile">'
	...
	*/
	function column_links( $item ) {
		...
		$out = '';
		$out .= '<a href="?page=domain-check-ssl-watch&domain_check_ssl_search=' . $item['domain_url']  . $paged . $order . '" alt="Refresh" title="Refresh">'
		...
		$out .= '<a href="?page=domain-check-ssl-watch&domain_check_ssl_delete=' . $item['domain_url']  . $paged . $order . '" alt="Delete" title="Delete">'
