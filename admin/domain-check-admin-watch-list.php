 ...
 * @since 0.1.0
 */
class DomainCheck_Watch_List extends WP_List_Table {
	/**
	* Constructor.
	*/
	...
	*/
	public static function get_domains( $per_page = 50, $page_number = 1 ) {
		global $wpdb;
		$sql = 'SELECT * FROM ' . DomainCheck::$db_prefix . '_domains WHERE domain_watch > 0';
	...
	*/
	public static function record_count() {
		global $wpdb;
		$sql = "SELECT COUNT(domain_id) FROM " . DomainCheck::$db_prefix . '_domains WHERE domain_watch > 0';
		...
		if ($item['domain_expires']) {
			...
			if ($days_flat < 60) {
				if ($item['status'] == 2) {
					...
					$out .= '&nbsp;&nbsp;<a href="' . DomainCheckLinks::ssl($item['domain_url']) . '" class="button" target="_blank">Backorder</a>';
				}
			}
		} else {
			$out = 'Unknown';
